...
SH
  # Backend seam: teardown sources fm-backend-lib.sh, which sources the tmux/herdr
  # libs from its own dir. Symlink the real libs so teardown loads them; the
  # fm_be_kill_window calls are 2>/dev/null || true, so no live tmux is needed.
  for lib in fm-backend-lib.sh fm-tmux-lib.sh fm-herdr-lib.sh; do
...
fm_tasks_axi_compatible() { return 1; }
SH
  # Backend seam libs (teardown sources fm-backend-lib.sh + its tmux/herdr deps).
  for lib in fm-backend-lib.sh fm-tmux-lib.sh fm-herdr-lib.sh; do
    ln -s "$ROOT/bin/$lib" "$fake/bin/$lib"
  done
