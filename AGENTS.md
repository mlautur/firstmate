...
state/               volatile runtime signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .seen-* .hb-surfaced-* .agent-status-* .last-* .heartbeat-streak   watcher internals; never touch (.agent-status-* remembers each window's last herdr agent_status level so a turn-end is synthesized once per transition)
