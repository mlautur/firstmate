...
# the env is set when the agent starts; routed through the backend seam so it
# works under both the tmux and herdr crew backends, and the brief sleep lets the
# export land before the launch text.
fm_be_run_cmd "$T" "export GOTMPDIR=$TASK_TMP/gotmp"
sleep 0.3
