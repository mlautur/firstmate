...
# Busy signatures per harness (claude/codex: "esc to interrupt"; opencode: "esc
# interrupt"; pi: "Working..."; grok: "Ctrl+c:cancel") are owned by the backend seam
# now: the busy determination in the staleness loop reads fm_be_agent_status, whose
# tmux backend applies the OR-ed regex (overridable via FM_BUSY_REGEX). See
# bin/fm-tmux-lib.sh.
