...
The tmux->herdr migration is landing in slices: the crew lifecycle and supervision paths now drive crew panes through the seam - `fm-spawn` (window create, `treehouse get`, and brief send), `fm-teardown` (window close), `fm-watch` (pane reads plus turn-end and stale wake decisions), and the away-mode daemon (`fm-supervise-daemon.sh`, pane-alive checks, supervisor-target discovery, and meta-driven window lookup) - so selecting `herdr` runs the claude crew through it, with the returned herdr pane handle stored in `state/<id>.meta window=`.
...
With no per-task hook under herdr, `fm-watch` derives the turn-end from the native `agent_status` level read through `fm_be_agent_status` (never the edge-triggered `herdr wait`): it synthesizes the `state/<id>.turn-ended` marker once per transition into a resting state (`done`, or the `working`->`idle` boundary), and a herdr-native `blocked` crew - which surfaces through its own status channel - is treated as neither a turn-end nor a stale pane.
...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel'   # busy-pane signatures applied by the tmux backend; the watcher and daemon read busy state through the seam
